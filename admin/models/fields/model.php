...
* @subpackage	Form
*/
class SerialsCkFormFieldModel extends JFormFieldGroupedList
{
	/**
	* Foreign Key Filter. 
	*
	...
	*/
	protected function getGroups()
	{
		...
		$listGroups = SerialsHelperForm::getElementGroups($this->element);
		...
		$items = SerialsHelperForm::getItems($this);
		...
		foreach($items as $item)
		{
			...
			{
				$val = SerialsHelper::parseValue($item, $labelKey);
				if ($val !== null)
	...
	*/
	protected function getOptions()
	{
		...
		if (!empty($items))
		foreach ($items as $item)
			$options[$item->$listKey] = SerialsHelper::parseValue($item, $labelKey);
	...
	*/
	public function prepareModel(&$model)
	{
		...
		$groups = SerialsHelperForm::getElementGroups($this->element);
...
if (!class_exists('JFormFieldModel')){ class JFormFieldModel extends SerialsCkFormFieldModel{} }
