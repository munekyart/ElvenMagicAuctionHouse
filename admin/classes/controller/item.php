...
* @subpackage	
*/
class SerialsCkClassControllerItem extends JControllerForm
{
	/**
	...
	* @var string
	*/
	protected $text_prefix = 'COM_SERIALS';
	...
	*/
	public function add()
	{
		...
		$this->applyRedirection($result, array(
			'com_serials.' . $this->view_list . '.default',
			'com_serials.' . $this->view_item . '.' . $this->getLayout('edit')
		));
	}
	...
	*/
	protected function applyRedirection($result, $redirections, $vars = array())
	{
		...
		switch($redirection)
		{
			...
			case 'stay':
				$this->setRedirect(SerialsHelper::urlRequest($vars));
				return;
				break;
		...
		//Filters followers
		$model = CkJModel::getInstance($this->view_list, 'SerialsModel');
		if ($model)
		{
	...
	*/
	public function delete()
	{
		...
		else
		{
			...
			{
				$result = true;
				$this->setMessage(JText::plural('SERIALS_ITEMS_SUCCESSFULLY_DELETED', count($cid)));
			}
			else
			{
	...
	public function redirect()
	{
		if ($this->redirect)
		{
			...
			if ($jinput->get('return') == 'json')
			{
				...
				SerialsClassAjax::responseJson(array(
	...
	*/
	protected function updateFileField($model, $fieldName, $options = array())
	{
		...
		try
		{
			$changedValue = SerialsHelperFile::updateFile($model, $fieldName, $options, $uploadDirectory);
		}
		catch(Exception $e)
		{
...
if (!class_exists('SerialsClassControllerItem')){ class SerialsClassControllerItem extends SerialsCkClassControllerItem{} }
