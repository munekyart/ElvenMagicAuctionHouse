...
* @subpackage	Classes
*/
class SerialsCkClassModelList extends JModelList
{
	/**
	* Data array
	*
	...
	*/
	...
	{
		$model = CkJModel::getInstance($this->view_item, 'SerialsModel');
	...
	*/
	public function getItems()
	{
		try
		{
			...
			{
				if (!$this->canAdmin())
					$error = JText::_('SERIALS_ERROR_INVALID_QUERY');
	...
	*/
	...
	{
		if ($item)
			return CkJModel::getInstance($this->view_item, 'SerialsModel');
	...
	*/
	...
	{
		$url = 'index.php?option=com_serials';
	...
	*/
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$globalParams = JComponentHelper::getParams('com_serials', true);
		...
		// FILTERS
			foreach($this->filter_vars as $var => $varType)
			{
				...
				{
					$date = SerialsHelperDates::timeFromFormat($value, $matches[1]);
					if ($date)
					{
	...
	*/
	public function updateXref($field, $values, $on, $id)
	{
		...
		$model = CkJModel::getInstance($this->getName(), 'SerialsModel');
		...
		else
		{
			...
			{
				$query = $db->getQuery(true);
				$query->update('#__serials_' . $this->getName())
...
if (!class_exists('SerialsClassModelList')){ class SerialsClassModelList extends SerialsCkClassModelList{} }
