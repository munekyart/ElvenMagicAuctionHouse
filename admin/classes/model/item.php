...
* @subpackage	Classes
*/
class SerialsCkClassModelItem extends JModelAdmin
{
	/**
	* Data array
	*
	...
	* @var string
	*/
	protected static $extension = 'serials';
	...
	*/
	public function __construct($config = array())
	{
		...
		// Load the ORM system
		$this->orm = SerialsClassModelOrm::getInstance($this);
	...
	*/
	public function canCheck($record)
	{
		...
		{			
			$this->setError(JText::_("SERIALS_TASK_RESULT_THE_USER_CHECKING_OUT_DOES_NOT_MATCH_THE_USER_WHO_CHECKED_OUT_THE_ITEM"));
			return false;			
		}
	...
	*/
	public function deletefiles($pks, $fileFields)
	{
		...
		$config	= JComponentHelper::getParams( 'com_serials' );
		...
		foreach($fileFields as $fieldName => $op)
		{
			...
			foreach($files as $fileObj)
			{
				...
				if (!SerialsHelperFile::removeFile($imagePath, $op))
					$errors = true;
			}
		}
	...
	*/
	...
	{
		$model = CkJModel::getInstance(ucfirst($table), 'SerialsModel');
	...
	*/
	public function getForm($data = array(), $loadData = true, $control = 'jform')
	{
		...
		$form->addRulePath(JPATH_ADMIN_SERIALS . '/models/rules');
	...
	*/
	...
	{
		$modelList = CkJModel::getInstance($this->getNameList(), 'SerialsModel');
		if ($modelList)
	...
	*/
	public function integrityRestrict($relation, &$pks, $labelKey, $parentLabelKey, $editLink)
	{
		...
		foreach($pks as $id)
		{
			...
			$children = SerialsHelper::getData($childModel, array(
				'filter' => array(
			...
			if (count($children))
			{
				...
				$model = SerialsHelper::componentModel($relation->foreignModelClass, true);
				...
				$msg = sprintf(JText::_('SERIALS_ERROR_IMPOSSIBLE_TO_DELETE_USED_IN'), ($itemName?$itemName:'item'), implode(', ', $childText));
	...
	*/
	protected function loadForm($name, $source = null, $options = array(), $clear = false, $xpath = false)
	{
		...
		// Get the form.
		JForm::addFormPath(JPATH_SERIALS . '/models/forms');
		JForm::addFieldPath(JPATH_SERIALS . '/models/fields');
	...
	*/
	public function loadRelation($name, &$item, $orm = array())
	{
		...
		$model = SerialsClassModelOrm::getModel($relation->foreignModelClass);
		if (!$model)
			return;
	...
	*/
	public function loadXref(&$item, $objectField, $xrefTable, $on, $key, $states = array(), $context = 'object.default')
	{
		...
		{
			$model = CkJModel::getInstance($xrefTable, 'serialsModel');
	...
	*/
	protected function populateState()
	{
		...
		if (defined('JDEBUG'))
			$_SESSION["Serials"]["Model"][$this->getName()]["State"] = $this->state;
	...
	*/
	...
	{
		$baseFolder = JPATH_SERIALS . '/fork/models/forms';
	...
	*/
	...
	{
		return SerialsClassModelOrm::queryDump($query, $output);
	}
	...
	*/
	public function toggle($fieldName, $pk = null, $max = 1)
	{
		...
		if (!$result)
		{
			JError::raiseWarning(1106, JText::sprintf("SERIALS_MODEL_IMPOSSIBLE_TO_TOGGLE", $fieldName));
			return false;
		}
...
if (!class_exists('SerialsClassModelItem')){ class SerialsClassModelItem extends SerialsCkClassModelItem{} }
