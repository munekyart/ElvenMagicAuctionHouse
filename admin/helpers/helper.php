...
/**
* Serials Helper functions.
*
* @package	Serials
* @subpackage	Helper
*/
class SerialsCkHelper
{
	/**
	...
	*/
	...
	{
		return SerialsHelperMenu::addSubmenu($alias, $view, $layout);
	}
	...
	*/
	public static function getActions($itemId = 0)
	{
		...
		foreach ($actions as $action)
			$result->set($action, $user->authorise($action, COM_SERIALS));
	...
	*/
	...
	{
		if (is_array($attribs))
			$attribs = SerialsHelperFile::getAttributesFromLegacy($attribs);

		return SerialsHelperFile::getFileUrl($path, $method, $attribs);
	}
	...
	*/
	public static function headerDeclarations()
	{
		...
		$baseSite = 'components/' . COM_SERIALS;
		$baseAdmin = 'administrator/components/' . COM_SERIALS;
		...
		SerialsHelperHtmlValidator::loadLanguageScript();
		SerialsHelperHtmlValidator::attachForm();
		...
		if ($app->isAdmin())
		{
			...
			self::addStyleSheet($baseAdmin, 'css/serials.css');
			self::addStyleSheet($baseAdmin, 'css/toolbar.css');

		}
		else if ($app->isSite())
		{
			self::addStyleSheet($baseSite, 'css/serials.css');
	...
	*/
	public static function loadFork($file)
	{
		...
		$file = preg_replace("#com_serials#", 'com_serials/fork', $file);
...
// Load the fork
SerialsCkHelper::loadFork(__FILE__);

// Fallback if no fork has been found
if (!class_exists('SerialsHelper')){ class SerialsHelper extends SerialsCkHelper{} }
